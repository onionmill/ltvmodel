...
```js
const A = Math.pow(10,-16.493), B = 5.688, GEN = Date.UTC(2009, 0, 3); // Bitcoin genesis
function trend(t) { return A * Math.pow((t - GEN) / DAY, B); }
```

Stress price = `band × trend(t)` where `band ≈ 0.42` (power law floor).
